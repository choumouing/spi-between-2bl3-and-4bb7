...
void uart_sbus_init (uart_index_enum uart_n, uint32 baud, uart_tx_pin_enum tx_pin, uart_rx_pin_enum rx_pin)
{
    ...
    uint32                      divSetting_int                  = (UART_FREQ * 2) / targetFreq;
    uint32                      divSetting_float                = (uint32)((double)((UART_FREQ * 2) - divSetting_int * targetFreq) / (double)targetFreq * 32.0f);
    ...
    g_stc_uart_config.oversample        = oversample_num;
    ...
    Cy_SysClk_PeriphSetFracDivider(CY_SYSCLK_DIV_24_5_BIT, (uint8)uart_n, (divSetting_int - 1), divSetting_float);
